...
/**
 * Provides a form for reverting a Disable periode entity revision.
 *
 * @ingroup prise_rendez_vous
 */
class DisPeriodEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'dis_period_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Disable periode entity: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Disable periode entity %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\prise_rendez_vous\Entity\DisPeriodEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(DisPeriodEntityInterface $revision, FormStateInterface $form_state) {
